...
impl Display for FileType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let icon: &str = match self {
            Self::File => "\u{f016}".into(),        // 
            Self::Directory => "\u{f115}".into(),   // 
            Self::Symlink => "\u{f481}".into(),     // 
            Self::BlockDevice => "\u{fc29}".into(), // ﰩ
            Self::CharDevice => "\u{e601}".into(),  // 
            Self::FiFo => "\u{f731}".into(),        // 
            Self::Socket => "\u{f6a7}".into(),      // 
        };
...
//             Self::Directory => "\u{f115}".into(),   // 
//             Self::File => "\u{f016}".into(),        // 
//             Self::Pipe => "\u{f731}".into(),        // 
//             Self::Socket => "\u{f6a7}".into(),      // 
//             Self::Executable => "\u{f489}".into(),  // 
//             Self::SymlinkDir => "\u{f482}".into(),  // 
//             Self::SymlinkFile => "\u{f481}".into(), // 
//             Self::DeviceChar => "\u{e601}".into(),  // 
//             Self::DeviceBlock => "\u{fc29}".into(), // ﰩ
...
//         // TODO: Handle all the other kinds
//         let mut filetype = FileType::File;
//         if p.is_dir() {
//             filetype = FileType::Directory;
//         } else if p.is_symlink() {
//             if p.symlink_metadata().unwrap().is_dir() {
//                 filetype = FileType::SymlinkDir
//             } else {
//                 filetype = FileType::SymlinkFile
//             }
//         }
//
//         let mut icon = filetype.get_icon();
//
//         if filetype == FileType::File {
//             let filename = p.file_name().unwrap().to_string_lossy().to_string();
//             if let Some(ic) = get_default_icons_by_name().get(&filename) {
//                 icon = ic.to_string();
//             } else {
//                 let extension = filename.split('.').last().unwrap_or("").to_string();
//                 if let Some(ic) = get_default_icons_by_extension().get(&extension) {
//                     icon = ic.to_string()
//                 }
//             }
//         }
...
//
// impl File {
//     pub fn to_string(&self, cli: &Cli) -> String {
//         let mut name = if cli.full {
//             String::from(self.path.to_string_lossy())
//         } else {
//             String::from(self.path.file_name().unwrap().to_string_lossy())
//         };
//
//         if self.filetype == FileType::SymlinkDir || self.filetype == FileType::SymlinkDir {
//             if let Ok(target) = read_link(&self.path) {
//                 name.push_str(
//                     format!(" -> {}", target.file_name().unwrap().to_string_lossy()).as_str(),
//                 );
//             } else {
//                 name.push_str(" \u{2715} ")
//             }
//         }
//
//         format!(
